    ...
    <div className="max-w-[1240px] mx-auto px-4 md:px-8">
      <section className="flex flex-col md:grid md:grid-cols-[1fr_auto] md:items-end gap-3 md:gap-8 pb-4 md:pb-6 border-b border-line mb-6 md:mb-8">
        <div>
          ...
          <h1 className="title-display text-[30px] md:text-[44px]">Status</h1>
        </div>
        <ManualSyncButton
...
function SyncRunItem({ run }: { run: SyncRunRow }) {
  ...
  return (
    <li className="flex flex-col gap-2 md:grid md:grid-cols-[180px_140px_auto_1fr] md:gap-4 py-3 border-b border-line-soft md:items-start">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 md:contents">
        ...
        <span
          className={`font-mono text-[10px] uppercase tracking-[0.14em] px-2 py-0.5 border rounded-sm md:self-start ${outcomeCfg.cls}`}
        >
          {outcomeCfg.label}
        </span>
      </div>
      <div className="min-w-0">
        ...
        {run.errorMessage ? (
          <p className="font-mono text-[11px] text-warn mt-1 break-words md:truncate" title={run.errorMessage}>
            {run.errorMessage}
          </p>
        ) : null}
      </div>
