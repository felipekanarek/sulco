...
**Última atualização**: 2026-05-04 (Inc 8 entregue — refator UX dos filtros + 5 filtros novos)
...
#### Incremento 36 — Otimizar filtros pesados: pivot record_formats + index composite com ORDER BY

Pós-deploy do Inc 8 (refator dos filtros + 5 novos), Felipe testou em
prod com filtros aplicados + hard refresh: ~3k rows lidas por
navegação. Diagnóstico EXPLAIN: `queryCollection` usa
`records_user_archived_imported_idx` pra ORDER BY DESC + LIMIT 50;
quando filtros são restritivos (poucos matches) o planner percorre
todas as ~2.6k records do user procurando 50 hits.

**Decisões pré-acordadas**:
- 4 indexes single-column criados em prod (year/country/label/
  shelf_location) como mitigação parcial. Planner ainda prefere
  imported_idx por causa do ORDER BY — ganho real menor que esperado.
- Caso pior identificado: filtro `format=LP` (2243 records) usa
  OR-de-LIKE × 4 patterns por token selecionado. Index single-column
  em format não ajuda (composite Discogs).

**Ângulos a considerar no spec**:
1. **Pivot `record_formats(record_id, token)`** análogo a `record_genres`
   (Inc 35). Filter SQL troca OR-de-LIKE por
   `id IN (SELECT record_id FROM record_formats WHERE token IN (?))`.
   Hooks de write em apply-update.ts/archive.ts (paralelo a record_genres).
2. **Index composite `(user_id, archived, year, imported_at)`** força
   planner a usar year como driver mantendo ORDER BY natural. Mesma
   ideia pra country/label se medições mostrarem necessário.
3. **Considerar bound em LIMIT** com paginação: hoje já tem (Inc 22),
   mas explorar reduzir reads quando filtro restritivo retorna < 50.

**Estimativa**: ~2-3h speckit. Schema delta: 1 pivot + N indexes.
