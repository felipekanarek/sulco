...
- **034** — Cobertura de testes retroativa (Inc 37) · 2026-05-05 · `specs/034-retroactive-test-coverage/` · aplica retroativamente Princípio VI (Constitution 1.3.0) sobre 10 features shippadas sem testes específicos (Inc 23-32). 3 tiers entregues: **Tier 3 helpers puros** — 3 unit tests (`normalize-text`, `compute-record-search-text`, `diff-vocab-arrays`) com 52 cases cobrindo diacríticos pt-BR + Unicode universal + idempotência + edge cases; **Tier 1 AUTHOR proteção** — 6 integration tests cobrindo `sync-preserves-author-fields` estendido com pivots Inc 35 (record_genres/record_styles/track_moods/track_contexts em INSERT/UPDATE/REAPARIÇÃO/CASCADE), `archive-record-author-preserved`, `delete-set-preserves-tracks`, `update-record-status`, `update-record-author-fields`, `update-track-curation` — 38 cases cobrindo caminho feliz + ownership-fail (u2 → u1) + Zod rejection + Princípio I preservation; **Tier 2 equivalence** — 4 integration tests cobrindo `buildCollectionFilters` (16 cases, 1 it() por filtro com seed determinístico de 5 records), `applyVocabDelta` (Inc 33 UPSERT increment + clamp), `applyPivotDelta` (Inc 35 onConflictDoNothing + DELETE seletivo + filtro empty), `cache-user` (pass-through pós-Inc 23 follow-up). **Mock pattern uniforme** (Clarification Q2): `vi.doMock('@/db', ...)` + `vi.doMock('@/lib/auth', ...)` + `vi.doMock('next/cache')` + `vi.doMock('@/lib/cache')`. Helper compartilhado novo: [tests/helpers/seed-collection.ts](tests/helpers/seed-collection.ts) cria 2 users + 5 records + 5 tracks + 1 set + pivots + user_facets + user_vocab populados. Refator preventivo: `tokenizeFormat` movido pra [src/lib/format-tokens.ts](src/lib/format-tokens.ts) já em Inc 36 (Princípio III single source). **`@vitest/coverage-v8` instalado + script `test:coverage` + baseline registrado em [coverage-baseline.md](specs/034-retroactive-test-coverage/coverage-baseline.md)** (Clarification Q1=A): 286 testes total (164→+122), arquivos críticos cobertos: text.ts/format-tokens.ts/cache.ts/pivot-helpers.ts 100%, archive.ts 96.72%, user-vocab.ts 90.66%, apply-update.ts 87.33%, collection.ts 72.43%. Threshold gate fica como Inc 38 candidato. **Zero touch em src/** (FR-007 + Princípio II preservados — `git diff main -- src/` retorna 0 linhas). Princípios I/II/III/IV/V/VI todos OK. Tempo real ~3h vs estimativa 6-8h (pattern uniforme + helper compartilhado encurtaram). test-db.ts estendido com record_genres/record_styles/record_formats/track_moods/track_contexts/user_vocab/user_facets para suportar Tier 1+2.
- **033** — Pivot record_formats + index composite (Inc 36) · 2026-05-04 · `specs/033-format-pivot-composite-idx/` · resolve gargalo de reads identificado em prod pós-Inc 8: filtros restritivos custavam ~3k rows lidas/navegação porque planner usava `records_user_archived_imported_idx` pra atender `ORDER BY imported_at DESC LIMIT 50` e percorria todo o index quando filtros encontravam <50 matches rápido. **Frente A — pivot `record_formats(record_id, token)`** análoga a Inc 35: PK composta + index reverso `(token, record_id)` + FK ON DELETE CASCADE. Substitui OR-de-LIKE × 4 patterns (Inc 8 follow-up) por `id IN (SELECT record_id FROM record_formats WHERE token IN ?)` em `buildCollectionFilters` em [src/lib/queries/collection.ts](src/lib/queries/collection.ts). EXPLAIN prod confirmou: `SEARCH record_formats USING COVERING INDEX record_formats_token_idx + LIST SUBQUERY + BLOOM FILTER`. **Frente B — index composite `records_user_archived_year_imported_idx (user_id, archived, year, imported_at DESC)`** força planner a usar year como driver mantendo ORDER BY natural. EXPLAIN prod confirmou: `SEARCH records USING COVERING INDEX records_user_archived_year_imported_idx (user_id=? AND archived=? AND year=?)` (ANTES: `SEARCH records USING INDEX records_user_archived_imported_idx (user_id=? AND archived=?)` que percorria todos os 2587 records). **Frente C — hooks de write paralelos a Inc 35**: `applyDiscogsUpdate` em [src/lib/discogs/apply-update.ts](src/lib/discogs/apply-update.ts) ganha 3 chamadas `applyPivotDelta(recordFormats, ..., tokenizeFormat(release.format), ...)` paralelas a record_genres/record_styles em INSERT (linha ~95), UPDATE com diff via `diffVocabArrays` (linha ~191) e REAPARIÇÃO (linha ~152). `archiveRecord` NÃO toca pivot — filter `archived=0` em queries impede pivot entries de archived aparecerem; FK CASCADE cobre delete físico raríssimo (mesmo padrão Inc 35). `applyPivotDelta` em [src/lib/pivot-helpers.ts](src/lib/pivot-helpers.ts) estendido: tipo `PivotTable` ganha `recordFormats`, `valueColumn` ganha `'token'`. **Backfill** via novo `scripts/_backfill-record-formats.mjs` (mesmo pattern `_backfill-pivot-tables.mjs` Inc 35): DELETE global + INSERT batched chunks 500 via `db.batch(stmts, 'write')`. Local: 5791 entries em 1741 records (3.3 tokens/record média). Prod: **8661 entries em 39 tokens distintos** (top: Vinyl=2575, LP=2243, Album=1743, Stereo=429, Compilation=387, Reissue=237, 7"=235, 33⅓ RPM=207, Single=130, Mono=115). **Migration prod aplicada via `@libsql/client db.batch`** atômico (CREATE TABLE + 2 indexes) ANTES do code deploy — ordem crítica documentada (code novo procura pivot e falharia até backfill rodar). Princípios I (pivot é zona SYS materializada — derivada de `records.format` que é DISCOGS), II (RSC + Server Actions intactos), III (1 tabela + 1 index composite + reuso de `applyPivotDelta`), IV (pivot derivada e recomputável via backfill, indexes aditivos), V (backend puro — UI inalterada) todos OK. Q1 do prompt original resolvido: composite apenas pra year (country/label/shelf ficam pra Inc 36b se monitoring revelar). Q2: NÃO drop os 4 indexes single-column do Inc 8 follow-up (cobrem queries sem ORDER BY). Reversível por revert + DROP TABLE + DROP INDEX restaurando comportamento Inc 8.
- **032** — Refator UX dos filtros + 5 filtros novos (Inc 8) · 2026-05-04 · `specs/032-filter-rework-and-new-fields/` · 3 frentes em 1 release. **Frente A — UX rework genres/styles**: lista expandida com `COLLAPSED_COUNT=10` substituída por picker buttons compactos (Q2=A); `<FilterPicker>` genérico chip-based em [src/components/filter-picker.tsx](src/components/filter-picker.tsx) com busca interna ativa quando ≥20 entries (Q3=B); desktop popover absoluto + mobile bottom sheet via `<MobileDrawer>` (primitiva Inc 009); ESC + click outside fecham. **Frente B — 5 filtros novos**: format, shelf, year, country, label. Single-column queries em `records.format`/`country`/`label`/`year`/`shelf_location` (zero schema delta, todas as colunas já existem). `buildCollectionFilters` em [src/lib/queries/collection.ts](src/lib/queries/collection.ts) ganha 5 conditions: country/label/shelf/year usam `IN (?)`; **format usa OR posicional de 4 LIKE patterns** (`= ?` OR `LIKE ?, %` OR `LIKE %, ?` OR `LIKE %, ?, %`) pra evitar falsos positivos em string composta Discogs ("Vinyl, LP, Album, Stereo"). URL search params multi-value (`format=LP&format=7"`); `<FilterBar>` ganha 10 props novas (5 selected + 5 available); `page.tsx` passa filtros pro queryCollection + paginator preserva params. **Frente C — picker de Ano**: `<YearFilterPicker>` em [src/components/year-filter-picker.tsx](src/components/year-filter-picker.tsx) com chips de anos individuais (busca interna `inputMode="numeric"` >20). Helper novo `listUserYears(userId)` faz `SELECT DISTINCT year` cached cross-request via `cacheUser`. Decisão dimensional confirmada na iteração com Felipe pós-deploy: **década (60s/70s/...) ficou ruim** — o real era multi-select de ano. **Schema delta**: `user_vocab.kind` CHECK constraint removido (Q4=C — recreate atomic via DDL batch preservando entries existentes); kinds estendidos pra 8 valores (`genres/styles/moods/contexts/shelves/formats/countries/labels`). Q5=A — strings vazias filtradas como NULL na agregação. **Materialização do vocabulário**: `listUserFormats`/`listUserCountries`/`listUserLabels` wrappers sobre `listVocab` (kinds novos populados via user_vocab Inc 33), todos cacheados cross-request via `cacheUser`. Format **tokenizado** (Discogs traz "Vinyl, LP, Album, Stereo" como string única; pickers mostram tokens base). Helper `tokenizeFormat` em [src/lib/discogs/apply-update.ts](src/lib/discogs/apply-update.ts) splita por vírgula nos hooks (insert/update path com diff via `diffVocabArrays`/reaparição via re-incremento) + em `archive.ts` (bulk decrement de tokens) + em `_repopulateVocab` em [src/lib/queries/user-facets.ts](src/lib/queries/user-facets.ts) via novo `_aggregateFormatTokenCounts`. **Backfill `_backfill-user-vocab-extended.mjs`**: zera + recria 8 kinds via `db.batch(stmts, 'write')` em chunks de 500 (mesmo pattern Inc 35). Aplicado em prod 2× (1ª com format composto, 2ª pós-tokenização revelou 39 tokens base — LP=2243, 7"=235, 12"=37, 10"=68 — vs 136 strings compostas). **Migration prod**: recreate `user_vocab` sem CHECK preservando 211→712 entries existentes via `INSERT INTO user_vocab_new SELECT ... + DROP + RENAME`. Aplicada via `@libsql/client` direto antes do code deploy. **Otimização de cache** (follow-up pós-Felipe testou e viu +77k reads): 7 listUser* helpers (genres/styles/shelves/formats/countries/labels/years) wrappados em `cacheUser` cross-request (TTL 300s, tag `user:${userId}` invalidada em writes — Inc 23 pattern). Eliminado `react.cache` (request-scoped insuficiente cross-render). **4 indexes single-column**: `(user_id, archived, year)`, `(..., country)`, `(..., label)`, `(..., shelf_location)` aplicados em prod via `db.batch` (mesmo pattern Inc 35). Format fica de fora — composito Discogs requer pivot table (Inc 36 no roadmap). Princípios I (filtros são leitura, vocab é zona SYS materializada), II (RSC + cache server), III (1 schema delta — recreate user_vocab CHECK + 4 indexes single-column), IV (vocab Inc 33 preservado, recreate atomic), V (mobile bottom sheet em todos os pickers + tap targets ≥44px) todos OK. Estimativa pós-real: ~4-5h speckit + iteração 2 (decade→year) + iteração 3 (cache + indexes).
- **031** — Excluir set (Inc 30) · 2026-05-03 · `specs/031-delete-set/` · resolve gap funcional do produto: DJ não conseguia remover sets criados (de teste, duplicados ou ultrapassados). Hard-delete via Server Action `deleteSet` em [src/lib/actions.ts](src/lib/actions.ts) com ownership check (`WHERE userId = user.id`) + cascade FK existing em `set_tracks.setId` (linha 215 do schema, sem schema delta). Tracks/records permanecem 100% intactos com curadoria autoral preservada. Client component `<DeleteSetButton>` em [src/components/delete-set-button.tsx](src/components/delete-set-button.tsx) usa `window.confirm` nativo (fullscreen mobile iOS/Android, Princípio V) com texto explícito sobre nome do set + preservação das faixas + irreversibilidade; `useTransition` + `router.push('/sets')` pós-sucesso; disabled durante isPending; mensagem de erro inline com auto-dismiss 5s (mesmo pattern Inc 19). Posicionamento: header de `/sets/[id]/page.tsx` ao lado de "Editar set" + header de `/sets/[id]/montar/page.tsx` ao lado de `<EditSetModal>` (Inc 16) — paridade UX entre as 2 rotas. Sem schema delta, sem migration prod. Princípios I/II/III/V todos OK; **Princípio IV** registra exceção justificada — hard-delete em sets é decisão de produto consciente porque sets são metadata curatorial criada e descartável pelo DJ, diferente de records.archived que protege curadoria contra sync externo. Reversível por revert (sets físicamente apagados não recuperáveis via UI; backup do banco se necessário, escala atual aceita).
- **030** — Pivot tables + sort indexes (Inc 35) · 2026-05-03 · `specs/030-filter-pivots-and-sort-indexes/` · ataca os 4 gargalos materiais identificados via EXPLAIN QUERY PLAN em prod (sessão investigação 2026-05-03 pós-Inc 34): filtros multi-select genres/styles na home (~10-15k rows lidas via `EXISTS json_each`), filtros moods/contexts em `/montar` (~10k rows), TEMP B-TREE sort `imported_at` em `/`, idem `archived_at` em `/status`. **Schema delta**: 4 tabelas pivot novas (`record_genres`, `record_styles`, `track_moods`, `track_contexts`) com PK composta `(fk_id, value)` + index reverso `(value, fk_id)` cobre lookup direto sem scan json_each. FK ON DELETE CASCADE. **+ 2 indexes auxiliares** em records: `records_user_archived_imported_idx (user_id, archived, imported_at DESC)` e `records_user_archived_archivedat_idx (user_id, archived, archived_at DESC)` eliminam TEMP B-TREE sort. **Helper novo** `applyPivotDelta` em `src/lib/pivot-helpers.ts` reutilizável pelos 4 callsites: filtra empty/whitespace + DELETE removidos batched + INSERT added com `onConflictDoNothing`. **Refator de queries**: `buildCollectionFilters` (collection.ts) substitui `EXISTS json_each(records.genres)` por `IN (SELECT record_id FROM record_genres WHERE genre IN ?)`; `queryCandidates` (montar.ts) faz mesmo padrão pra moods/contexts (1 cláusula por termo preserva semântica AND). **Hooks de write**: `updateTrackCuration` (actions.ts) ganha 2 chamadas `applyPivotDelta` paralelas às chamadas Inc 33 a `applyVocabDelta` (track_moods + track_contexts); `applyDiscogsUpdate` (discogs/apply-update.ts) populates record-level pivots no INSERT path quando `created=true`, faz diff no UPDATE path, e re-popula record + track-level pivot na reaparição (wasArchived=true→false). `archiveRecord` NÃO toca pivot — filtros base têm `WHERE archived=0`; FK cascade cobre delete físico raríssimo. **Backfill** via `scripts/_backfill-pivot-tables.mjs` com `db.batch()` em chunks de 500 (otimizou de 1h40min sequencial pra ~1min em libsql remoto): populou 16.371 entries em prod (4575 record_genres + 4522 record_styles + 7168 track_moods + 106 track_contexts). **Migration prod aplicada via turso db shell** após code deploy (gate verificável: `SELECT COUNT(*)` nas 4 tabelas retornou >0). **EXPLAIN prod confirmado**: `LIST SUBQUERY + SEARCH record_genres USING COVERING INDEX record_genres_genre_idx + BLOOM FILTER` (sem json_each); listagem default usa `records_user_archived_imported_idx` sem TEMP B-TREE; filtro mood usa `track_moods_mood_idx`. **Ganho medido em prod**: 3 ações UI (hard refresh `/` + filtro Rock + limpar) consumiram 5.235 rows lidas — ~1.745 rows/ação vs ~10-15k rows/filtro pré-Inc 35 = **redução ~85%** (não os 99% teóricos projetados; diferença vem do overhead interno do Turso ~5-6× sobre rows retornadas, não eliminável sem trocar banco). Isso muda a app de "estoura cota em 1 sessão" pra "centenas de sessões cabem no free tier". **Princípios I/II/III/IV/V todos OK**. Reversível por revert + 4 DROP TABLE + 2 DROP INDEX restaurando comportamento Inc 32-34.
- **029** — Drop colunas `*Json` em `user_facets` (Inc 34) · 2026-05-03 · `specs/029-drop-user-facets-json/` · cleanup pós-Inc 33: as 5 colunas JSON em `user_facets` (`genresJson`/`stylesJson`/`moodsJson`/`contextsJson`/`shelvesJson`) ficaram mortas após Inc 33 (todos os 5 readers de vocabulário migraram pra `listVocab` que lê de `user_vocab`). **Schema delta**: 5 colunas removidas via `ALTER TABLE DROP COLUMN`; tabela `user_facets` enxuga de 12 → 7 campos (PK `userId` + 4 records counters + `tracksSelectedTotal` + `updatedAt`). **Tipo `UserFacets`** em [src/lib/queries/user-facets.ts](src/lib/queries/user-facets.ts) reflete schema enxuto (12 → 7 campos); `getUserFacets` retorna apenas counters preservados, sem `parseJsonArray`. **`recomputeFacets`** simplificado: `Promise.all` reduzido de 7 → 2 chamadas (apenas `aggregateCounts` + `aggregateTracksSelected`); INSERT/onConflictDoUpdate com 6 fields em vez de 12; mantém chamada a `_repopulateVocab(userId)` para drift correction de `user_vocab`. **`_repopulateVocab` self-contained**: agora agrega `genres`/`styles` inline via SELECT em records + reusa `_aggregateVocabCounts`/`_aggregateShelfCounts` do Inc 33; assinatura enxugada para `(userId)` (não recebe mais agregações via param). **Helpers privados deletados**: `aggregateFacet`, `aggregateVocabulary`, `aggregateShelves`, `parseJsonArray` (todos alimentavam exclusivamente as colunas JSON removidas). **Comments ajustados** em `montar.ts` (`listSelectedVocab` agora cita Inc 33/`user_vocab` em vez de `user_facets.moodsJson`) e header de `user-facets.ts` (referenciam apenas Inc 24/27/33/34 atuais). **Ordem de deploy invertida vs Inc 33**: code deploy primeiro, depois migration prod via `turso db shell sulco-prod` (código novo não depende das colunas; tabela velha tem colunas extras ignoradas pelo Drizzle SELECT que enumera apenas campos do schema TS). Smoke test em prod confirmou: `getUserFacets` retorna 7 colunas; pickers de moods/contexts/genres/styles populados via `listVocab`; `/sets/[id]/montar` carrega normalmente; queries de `recomputeFacets` esperadas com 5 SELECTs a menos por run no cron diário. Princípios I (zona SYS materializada — sem AUTHOR), II (RSC + Server Actions intactos), III (schema reflete realidade pós-Inc 33), IV (reversível por revert + 5 ALTER TABLE ADD COLUMN com defaults vazios), V (zero impacto UI mobile) todos OK. Inc 34 fecha o ciclo de cleanup pós-otimizações de reads (Inc 22-33). Próximas features voltam pra UX (Inc 30/31/29).
